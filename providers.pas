...
type
  ...
  TfrmProviders = class(TForm)
    ...
    procedure navCatalogClick(Sender: TObject; Button: TDBNavButtonType);
  private
...
procedure TfrmProviders.refreshGrid();
begin
   with conCatalog do
    begin
      if Connected then
...
procedure TfrmProviders.updateData(id : integer);
begin
      ...
      if Connected then
        begin
          with qryInfo do
            begin
              SQL.Clear;
              SQL.Text:='SELECT c.id, c.name, c.seller, c.email, c.email2, c.street, c.number, pc.postal_code, pc.state, pc.colony, pc.municipality, pc.city, c.phone, c.mobile, c.facebook, c.twitter, c.web, cu.currency as currencyN FROM `providers` AS c JOIN currencies AS cu ON (c.currency = cu.id) JOIN postal_codes AS pc ON (c.cp = pc.id) WHERE c.id = :qryID';
              ...
              if NOT EOF then
                begin
                  ...
                  edtApellido.Text:=FieldByName('seller').AsString;
                  cmbMoneda.Text:=FieldByName('email').AsString;
                  ...
                  edtGoogle.Text:=FieldByName('web').AsString;
...
//Crea el formulario
procedure TfrmProviders.FormCreate(Sender: TObject);
begin
  Operation:='E';
  session := loadConfig;
  frmProviders.Top:=250;
  frmProviders.Width:= screen.Width - 50;
  frmProviders.Height:=screen.Height - 250;
  pnlLeft.Width:=Trunc(frmProviders.Width/2);
  ...
  Query := 'SELECT `id` AS ID, `name` AS EMPRESA, `seller` AS CONTACTO, `email` AS EMAIL, `phone` AS TELEFONO, `mobile` AS CELULAR, `debt` AS DEUDA FROM `providers`';
  Where := '';
  Order := ' ID';
  Direction := ' ASC';
...
procedure TfrmProviders.imgBackResize(Sender: TObject);
var
...
begin
  ...
  lblMoneda.Top:=edtNombre.Top+edtNombre.Height+Alto1;
...
procedure TfrmProviders.navCatalogClick(Sender: TObject;
...
procedure TfrmProviders.FormChangeBounds(Sender: TObject);
begin
  if frmProviders.Active then
    begin
      frmProviders.Top:=125;
      frmProviders.Left:=25;
    end;
end;

// Se usa para traer los datos de la dirección cuando se selecciona un codigo postal
procedure TfrmProviders.dbcmbCPSelect(Sender: TObject);
begin
  edtEstado.Caption:='';
...
procedure TfrmProviders.btnNewClick(Sender: TObject);
begin
  Operation := 'A';
...
procedure TfrmProviders.btnQuitClick(Sender: TObject);
begin
    if Cambio then
      begin
...
procedure TfrmProviders.btnReportClick(Sender: TObject);
begin
...
function TfrmProviders.getCP(cp : Integer; colony : String) : Integer;
begin
  with conInfo do
    begin
      if Connected then
...
function TfrmProviders.getCurrency(currency : String) : Integer;
begin
  with conInfo do
    begin
      if Connected then
...
procedure TfrmProviders.btnSaveClick(Sender: TObject);
var
  first_name, last_name, email, email2, street, number, phone, mobile, facebook, twitter, google_plus : String;
  cp, currency : Integer;
begin
  if Cambio then
    begin
      ...
      if MessageDlg('Guardar Proveedor', '¿Desea guarar los datos del proveedor ' + edtNombre.Text + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
        begin
          with conInfo do
            begin
              if Connected then
                ...
                begin
                  with qryInfo do
                    begin
                      ...
                      if Operation = 'A' then
                        begin
                          SQL.Text:='INSERT INTO `providers` (name, seller, email, email2, street, number, phone, mobile, facebook, twitter, web, cp, currency) VALUES (:qryFirstName, :qryLastName, :qryEmail, :qryEmail2, :qryStreet, :qryNumber, :qryPhone, :qryMobile, :qryFacebook, :qryTwitter, :qryGooglePlus, :qryCp, :qryCurrency)';
                        ...
                        begin
                          SQL.Text:='UPDATE `providers` SET name = :qryFirstName, seller = :qryLastName, email = :qryEmail, email2 = :qryEmail2, street = :qryStreet, number = :qryNumber, phone = :qryPhone, mobile = :qryMobile, facebook = :qryFacebook, twitter = :qryTwitter, web = :qryGooglePlus, cp = :qryCp, currency = :qryCurrency WHERE id = :qryID';
          ...
          showmessage('El proveedor ha sido almacenado correctamente.');
          Operation:='E';
          Cambio := false;
          btnSave.Enabled:=false;
        end
...
procedure TfrmProviders.btnDeleteClick(Sender: TObject);
begin
  if Operation = 'A' then
    begin
      edtId.Text:='';
    ...
    begin
      if MessageDlg('Eliminar Proveedor', '¿Desea eliminar el proveedor ' + edtNombre.Text + '?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
        begin
          with conInfo do
            begin
              if Connected then
              ...
              if Connected then
                begin
                  with qryInfo do
                    begin
                      SQL.Clear;
                      SQL.Text:='DELETE FROM `providers` WHERE id = :qryID';
                      params.ParamByName('qryID').AsInteger:=StrToInt(edtId.Text);
                      ExecSQL;
                      SQLTransaction.Commit;
...
procedure TfrmProviders.dbgCatalogTitleClick(Column: TColumn);
var
  i : integer;
begin
...
procedure TfrmProviders.edtNombreChange(Sender: TObject);
begin
  Cambio := true;
...
procedure TfrmProviders.edtSearchButtonClick(Sender: TObject);
var
  i : Integer;
begin
  Query := 'SELECT `id` AS ID, `name` AS EMPRESA, `seller` AS CONTACTO, `email` AS EMAIL, `phone` AS TELEFONO, `mobile` AS CELULAR, `debt` AS DEUDA FROM `providers`';
...
procedure TfrmProviders.edtSearchChange(Sender: TObject);
var
  number : Integer;
begin
  if (edtSearch.Text <> '') then
    begin
      ...
      if number > 0 then
        Where := ' WHERE (ID = :qryID) OR (name LIKE CONCAT(''%'',:qrySearch,''%'')) OR (seller LIKE CONCAT(''%'',:qrySearch,''%'')) OR (email LIKE CONCAT(''%'',:qrySearch,''%''))  OR (phone LIKE CONCAT(''%'',:qrySearch,''%'')) OR (web LIKE CONCAT(''%'',:qrySearch,''%''))'
      else
        Where := ' WHERE  (name LIKE CONCAT(''%'',:qrySearch,''%'')) OR (seller LIKE CONCAT(''%'',:qrySearch,''%'')) OR (email LIKE CONCAT(''%'',:qrySearch,''%''))  OR (phone LIKE CONCAT(''%'',:qrySearch,''%'')) OR (web LIKE CONCAT(''%'',:qrySearch,''%''))';
...
procedure TfrmProviders.dbgCatalogCellClick();
begin
  if Cambio then
    begin
      if MessageDlg('Se perderán los cambios', '¿Desea continuar?', mtConfirmation, [mbYes, mbNo], 0) = mrYes then
